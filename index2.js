...
const liner = new lineByLine('./SKUs2.txt');
let line;
(async () => {
	...
	const getProductDetails = async (link, skuFromFile) => {
		...
		if (link) {
            ...
            fs.appendFileSync('Founds2.csv',`${skuFromFile}`);
            let oneImg;
            let multipleImg;
            if (image.length==1) {
                oneImg=image[0];
            ...
            const fileExists = fs.existsSync('data2.xls');
            if (!fileExists) {
            fs.writeFileSync('data2.xls', `${header}\n`);
            }

            fs.appendFileSync('data2.xls', `${line}\n`);
            await page.close();
